...
int ft_atoi(char *s) //Define a function that takes a string representation of a number and return an integer (the actual number)
{
	...
	while (*s) //Loop until the end of the string is reached
		res = res * 10 + *s++ - 48; //For the current character in `s`, multiply `res` by `10`, then add the ASCII value of the current character (subtracting `48` to get the actual digit), then assign the result to res. See #1
	return (res); //Return the actual digit of the string representation of an integer
}
...
void    ft_putnbr(unsigned int n) //Define a function that takes an integer. The return type is `void` because the function will print the character `res` to the STDOUT
{
    ...
    if (n >= 10) //Checks if `n` consists of more than `1` digit
...
void add_prime_sum(char *str) //Define a function that takes a string and prints to the STDOUT the `sum` of all prime numbers inferior or equal to a number represented by `char `str`
{
    ...
    if (is_prime(n)) //Checks if `n` itself is a prime number, if not, skip the loop
    {
    ...
    if (sum != 0) //Checks if `sum` is not `0`
        ft_putnbr(sum); //If not `0`, write `sum` to the STDOUT, otherwise nothing is printed
}
...
{
    if (argc == 2) //Perform operations only if argc is two, meaning there needs to be 1argument in addition to the program name
